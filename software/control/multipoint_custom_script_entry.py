...
def multipoint_custom_script_entry(multiPointWorker, time_point, current_path, coordinate_id, coordiante_name, i, j):
    ...
    print(
        "in custom script; t "
        + str(multiPointWorker.time_point)
        + ", location "
        + coordiante_name
        + ": "
        + str(i)
        ...
        if multiPointWorker.do_autofocus:
            ...
            if multiPointWorker.time_point == 0:
                ...
                config_AF = next(
                    (
                        config
                        for config in multiPointWorker.configurationManager.configurations
    ...
    # if z location is not included in the scan coordinates
        ...
        if multiPointWorker.do_reflection_af == False:
            ...
            if (
                ((multiPointWorker.NZ == 1) or Z_STACKING_CONFIG == "FROM CENTER")
                and (multiPointWorker.do_autofocus)
                and (multiPointWorker.FOV_counter % Acquisition.NUMBER_OF_FOVS_PER_AF == 0)
            ):
            ...
            if multiPointWorker.reflection_af_initialized == False:
                ...
                multiPointWorker.microscope.laserAutofocusController.move_to_target(
                    0
    ...
    if multiPointWorker.NZ > 1:
        ...
        if Z_STACKING_CONFIG == "FROM CENTER":
            multiPointWorker.navigationController.move_z_usteps(
                -multiPointWorker.deltaZ_usteps * round((multiPointWorker.NZ - 1) / 2)
            )
    ...
    for k in range(multiPointWorker.NZ):
        ...
        file_ID = (
            ...
            + str(i)
            + "_"
            + str(j if multiPointWorker.x_scan_direction == 1 else multiPointWorker.NX - 1 - j)
            + "_"
            + str(k)
        )
        ...
        multiPointWorker.signal_register_current_fov.emit(
            multiPointWorker.navigationController.x_pos_mm, multiPointWorker.navigationController.y_pos_mm
        )
        ...
        if multiPointWorker.multiPointController.abort_acqusition_requested:
            ...
            if multiPointWorker.navigationController.get_pid_control_flag(2) is False:
                ...
                multiPointWorker.navigationController.move_z_usteps(
                    -multiPointWorker.dz_usteps - _usteps_to_clear_backlash
                )
                multiPointWorker.wait_till_operation_is_completed()
            ...
            multiPointWorker.coordinates_pd.to_csv(
    ...
    if multiPointWorker.NZ > 1:
        ...
        if Z_STACKING_CONFIG == "FROM CENTER":
            if multiPointWorker.navigationController.get_pid_control_flag(2) is False:
                _usteps_to_clear_backlash = max(160, 20 * multiPointWorker.navigationController.z_microstepping)
                multiPointWorker.navigationController.move_z_usteps(
                    -multiPointWorker.deltaZ_usteps * (multiPointWorker.NZ - 1)
                    + multiPointWorker.deltaZ_usteps * round((multiPointWorker.NZ - 1) / 2)
            ...
            if multiPointWorker.navigationController.get_pid_control_flag(2) is False:
                _usteps_to_clear_backlash = max(160, 20 * multiPointWorker.navigationController.z_microstepping)
                multiPointWorker.navigationController.move_z_usteps(
                    -multiPointWorker.deltaZ_usteps * (multiPointWorker.NZ - 1) - _usteps_to_clear_backlash
                )
                multiPointWorker.wait_till_operation_is_completed()
                multiPointWorker.navigationController.move_z_usteps(_usteps_to_clear_backlash)
                multiPointWorker.wait_till_operation_is_completed()
