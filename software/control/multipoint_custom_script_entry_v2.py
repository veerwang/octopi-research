...
def perform_contrast_autofocus(worker, region_id):
    if (
        (worker.NZ == 1 or worker.z_stacking_config == "FROM CENTER")
...
def update_widget_z_level(worker, region_id):
    if worker.coordinate_dict is not None:
        ...
        try:
            worker.microscope.multiPointWidgetGrid.update_region_z_level(
                region_id, worker.navigationController.z_pos_mm
            )
        except:
...
def get_autofocus_config(worker):
    ...
    return next(
        (config for config in worker.configurationManager.configurations if config.name == configuration_name_AF)
    )
...
def capture_image_hardware_trigger(worker, config):
    if "Fluorescence" in config.name and ENABLE_NL5 and NL5_USE_DOUT:
        ...
        worker.microcontroller.send_hardware_trigger(
            control_illumination=True, illumination_on_time_us=worker.camera.exposure_time * 1000
        )
        return worker.camera.read_frame()
...
def process_and_save_image(worker, image, file_ID, config, current_path, current_round_images, i, j, z_level):
    ...
    image_to_display = utils.crop_image(
        ...
        round(worker.crop_height * worker.display_resolution_scaling),
    )
...
def save_image(worker, image, file_ID, config, current_path):
    if image.dtype == np.uint16:
        ...
        saving_path = os.path.join(
            current_path, f"{file_ID}_{config.name.replace(' ', '_')}.{Acquisition.IMAGE_FORMAT}"
        )
...
def move_z_stack_back_from_center(worker):
    ...
    if worker.navigationController.get_pid_control_flag(2) is False:
        worker.navigationController.move_z_usteps(
            -worker.deltaZ_usteps * (worker.NZ - 1)
            + worker.deltaZ_usteps * round((worker.NZ - 1) / 2)
        ...
        worker.navigationController.move_z_usteps(
            -worker.deltaZ_usteps * (worker.NZ - 1) + worker.deltaZ_usteps * round((worker.NZ - 1) / 2)
        )
        worker.wait_till_operation_is_completed()
    worker.dz_usteps = (
        worker.dz_usteps - worker.deltaZ_usteps * (worker.NZ - 1) + worker.deltaZ_usteps * round((worker.NZ - 1) / 2)
    )
